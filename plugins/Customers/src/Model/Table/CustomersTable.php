...
class CustomersTable extends Table
{
    ...
    public function getFranq($cliente_id = null){

      if(empty($cliente_id)){
        return false;
      }

      $getcontumer = $this->get($cliente_id);

      $data = $this->find('all', ['contain' => ['Franquicias','Offices'],'conditions' => ['Customers.rfc' => $getcontumer->rfc ]])->toArray();

      return $data;
    }

    public function beforeFind($event, $query, $options, $primary){
        $query->andWhere(['Customers.deleted' => '0']);
    }

    public function getList($conds = []){
        return $this->find('list')->where($conds)->toArray();
    }

    public function altList($conds = []){
        return [null => __('Seleccione...')] + $this->getList($conds);
    }
